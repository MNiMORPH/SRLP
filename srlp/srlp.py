...
class LongProfile(object):
    ...
    def __init__(self):
        ...
        self.b = None # Width and depth need not be resolved to compute
        ...
        #self.gravel_fractional_loss_per_km = None      ## commented out for sand-bed rivers SRLP, by NI.
        ...
        self.dQ_up_jcn = 0.
        # Constants for sand-bed rivers SRLP (set later), by NI:
        self.n = None # Manning's roughness
    ...
    def basic_constants(self):
        ...
        #self.tau_star_c = 0.0495       ## commented out for sand-bed rivers SRLP, by NI.
        #self.phi = 3.97 # coefficient for Wong and Parker MPM      ## commented out for sand-bed rivers SRLP, by NI.

    def set_D(self, D):
        self.D = D

    def set_Mannings_roughness(self, n):
        self.n = n 
    ...
    def sediment_lumped_constants(self): # bedload_ to sediment_lumped_constants, modified for sand-bed rivers SRLP, by NI.
...
##   BELOW function commented out for sand-bed rivers SRLP, by NI.
...
#                / ( (1-self.lambda_p) * self.B )
    ...
    def set_Qs_input_upstream(self, Q_s_0):
        ...
        self.S0 = np.sign(self.Q[0]) * self.sinuosity * \
                      ( np.abs(Q_s_0) /
                        ( self.k_Qs
                              * np.abs(self.Q[0])) )**(6/5.) # 6/5 for sand-bed rivers SRLP, by NI.
    ...
    def update_z_ext_0(self):
        """
        Give upstream cell the same width as the first cell in domain.
        Check GRLP if further info necessary -by NI.
        """
        # Only one segment: towards applying boundary condition upstream
        self.z_ext[0] = self.z[0] + self.S0 * self.dx_ext[0]

    def compute_coefficient_time_varying(self):  # modified after grlp v2.0.0 beta release, by NI.
        ...
        dzdx_0_16 = np.abs( (self.z_ext[2:] - self.z_ext[:-2]) \
                         / self.dx_ext_2cell )**(-1/6.)  # (-1/6) for sand-bed rivers SRLP, by NI.
        self.C1 = self.C0 * dzdx_0_16 * self.Q / self.B

    def network__compute_coefficient_time_varying(self):  # added after grlp v2.0.0 beta release, by NI.
        i=0
        # !!!!!!!!!!!!!!!!!!!!!
        ...
        for _idx in range(len( self.z_ext) ):
            ...
            ## This loop and all others where (-1/6) power exists were modified for sand-bed rivers SRLP, by NI and AW:
            ## Edited to be 0 if dz = 0
            dz = (self.z_ext[_idx][2:] - self.z_ext[_idx][:-2])
            dzdx_0_16 = np.zeros(len(dz)) # zeros by default and where dz = 0
            dzdx_0_16[dz != 0] = np.abs( dz[dz != 0] \
                        / self.dx_ext_2cell[_idx][dz != 0] )**(-1/6.)  # (-1/6) for sand-bed rivers SRLP, by NI.
            dzdx_0_16_list.append(dzdx_0_16)
    ...
    def set_x_bl(self, x_bl):  # earlier moving boundary update, added after grlp v2.0.0 beta release, by NI. 
        self.x_bl = x_bl
        self.x_ext[-1] = self.x_bl

    def set_bcr_Dirichlet(self):  # changed after grlp v2.0.0 beta release and modified to 5/3 for sand-bed rivers SRLP, by NI.
        # !!!C0!!!  -see same grlp function for further comments, by NI.

        if type(self.x_ext) is list:
            # !!!!!!!!!!!!!!!!!!!!!! see grlp function for further comments regarding the change, by NI.
            self.bcr = self.z_bl * self.C1[-1] / self.dx_ext_2cell[0][-1] * (
                         5/3. * ( 1/(self.x_ext[0][-1] - self.x_ext[0][-2]) )
                         +
                         self.dQ_ext_2cell[0][-1]/self.Q[-1]
                         * 1/self.dx_ext_2cell[0][-1]
                         )
        else:
            self.bcr = self.z_bl * ( self.C1[-1] / self.dx_ext_2cell[-1] * 5/3. \
                           * (1/self.dx_ext[-2] + 1/self.dx_ext[-1])/2. \
                           + self.dQ_ext_2cell[-1]/self.Q[-1] )

    def set_bcl_Neumann_RHS(self):  # changed after grlp v2.0.0 beta release and modified to 5/3 for sand-bed rivers SRLP, by NI.
        """
    ...
    def set_bcl_Neumann_LHS(self):  # changed after grlp v2.0.0 beta release and modified to 5/3 for sand-bed rivers, by NI.
        """
        Boundary condition on the left (conventionally upstream) side of the
        domain.
        ...
        if type(self.x_ext) is list:
            # !!!!!!!!!!!!!!!!!!!!!! see grlp function for further info regarding the change, by NI.
            self.right[0] = -self.C1[0] / self.dx_ext_2cell[0][0] * 5/3. \
                             * (1/self.dx_ext[0][0] + 1/self.dx_ext[0][1])
        else:
            self.right[0] = -self.C1[0] / self.dx_ext_2cell[0] * 5/3. \
    ...
    def evolve_threshold_width_river(self, nt=1, dt=3.15E7):
        ...
        for ti in range(int(self.nt)):
            ...
            for i in range(self.niter):
                ...
                # river too
            #    if self.gravel_fractional_loss_per_km is not None:  ## commented out for sand-bed rivers SRLP, by NI.
    ...
    def build_LHS_coeff_C0(self, dt=3.15E7):
        ...
        self.C0 = self.k_Qs * self.intermittency \
                    ...
                    * self.dt  # 5/6 for sand-bed rivers srlp, by NI.

    def build_matrices(self):  # modified after grlp v2.0.0 beta release and modfied to 5/3 for sand-bed rivers SRLP, by NI.
        """
    ...
    def network__build_matrix_inner(self):  # added after grlp v2.0.0 beta release, modified to 5/3 & (-1/6) for sand-bed rivers SRLP, by NI.
        """
        Build the tridiagonal matrix (LHS) and the RHS matrix for the solution
        """
        self.network__compute_coefficient_time_varying()  # below comments are not updated for SRLP, by NI.
        # !!!C0!!!
        # UPDATED WITH STRAIGHT self.dx_ext_2cell
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # THIS IS A TOTAL H ACK, BUT IT SHOULD ACTUALLY BUILD SUCH
        ...
        """ this part is not updated for sand-bed rivers SRLP, by NI.
        ...
        if len(self.upstream_segment_IDs) > 0:
            ...
            for _tribi in range( len(self.upstream_segment_IDs) ):
                ...
                # segments
                ## Again, edited to be 0 if dz = 0, by NI and AW.
                dz = self.z_ext[_tribi][0] - self.z_ext[_tribi][1]
                dzdx_0_16 = 0 # zeros by default and where dz = 0
                if dz != 0:
                    dzdx_0_16 = ( np.abs( dz )
                                / self.dx_ext[_tribi][0] )**(-1/6)  # (-1/6) for sand-bed rivers SRLP, by NI.
            ...
            ## Again, edited to be zero where dz = 0, by NI and AW.
            dz = self.z_ext[0][1] - self.z_ext[0][2]
            dzdx_0_16 = 0 # zeros by default and where dz=0
            if dz != 0:
                dzdx_0_16 = ( np.abs( dz )
                            / self.dx[0][dz != 0] )**(-1/6.)  # (-1/6) for sand-bed rivers SRLP, by NI.
    ...
    def analytical_threshold_width(self, P_xQ=None, x0=None, x1=None,
        ...
        self.P_a = 1 - 6*P_xQ/5. # beta  # changed to 6*P_xQ/5 for sand-bed rivers SRLP, by NI.  
    ...
    def analytical_threshold_width_perturbation(self, P_xQ=None, x0=None, x1=None,
        ...
        #self.P_a = 1 - 6*P_xB/5. # beta  # changed to 6*P_xQ/5 for sand-bed rivers SRLP, by NI.
        ...
        K = self.k_Qs * self.sinuosity * self.intermittency \
            ...
            * self.k_xQ / self.k_xB  # modified to (-1/6) for sand-bed rivers SRLP, by NI.
        P = self.P_xQ
        #print(P)
    ...
    def compute_Q_s(self):  # changed after grlp v2.0.0 beta release and modified for sand-bed rivers SRLP, by NI and AW.
        S = []
        Q_s = []
        
        # To make SRLP compatible with network or with single segments, by NI and AW.
        if type(self.z_ext) is not list:
            _local_z_ext = [self.z_ext]
        else:
            _local_z_ext = self.z_ext
        if type(self.dx_ext_2cell) is not list:
            _local_dx_ext_2cell = [self.dx_ext_2cell]
        else:
            _local_dx_ext_2cell = self.dx_ext_2cell
       
        # Now, with a uniform input style, use this new local variable, by NI and AW.
        for _z, _dx in zip(_local_z_ext, _local_dx_ext_2cell):
            S.append( np.abs( (_z[2:] - _z[:-2]) / _dx) / self.sinuosity )
            Q_s.append(
                -np.sign( _z[2:] - _z[:-2] ) \
                * self.k_Qs * self.intermittency * self.Q * S[-1]**(5/6.)
                ) # 5/6 for sand-bed rivers SRLP, by NI.
        self.S = np.mean(S, axis=0)
        self.Q_s = np.mean(Q_s, axis=0)

        # # old way copied below, by NI.
        # self.S = np.abs( (self.z_ext[2:] - self.z_ext[:-2]) /
        #                 (self.dx_ext_2cell) ) / self.sinuosity
        # self.Q_s = -np.sign( self.z_ext[2:] - self.z_ext[:-2] ) \
        #           * self.k_Qs * self.intermittency * self.Q * self.S**(5/6.)

        # # old, non-network way -AW
    ...
    def compute_channel_width(self): #  calculation based on Eqn 11 (of the current SRLP paper draft), by NI.
        self.b = self.n * (self.rho * self.g)**(5/3.) \
                            * ((1+self.epsilon) * self.tau_crit_bank)**(-5/3.) \
                            * self.Q * self.S**(7/6.)

    def compute_flow_depth(self):  # calculation based on Eqn 7 (of the current SRLP paper draft), by NI.
        self.h = (1+self.epsilon) * self.tau_crit_bank \
                        / (self.rho * self.g * self.S)
...
# Following functions were deleted for the first release of SRLP by NI and AW (at Dogwood):
...
# Adding them again after grlp v2.0.0 beta release, by NI.

    def compute_diffusivity(self):  # modified to 5/6 and (-1/6) for sand-bed rivers SRLP, by NI.
        """
        Compute diffusivity at each point along valley.
        From linearized version of threshold width equation.
        """
        self.compute_Q_s()
        self.diffusivity = (5./6.) * self.k_Qs * self.intermittency * self.Q * self.S**(-1./6.) \
            / self.sinuosity**(5./6.) / self.B / (1. - self.lambda_p)
    ...
    def compute_z_gain(self, period, nsum=100):  # modified to 6/5 for sand-bed rivers SRLP, by NI.
        """
...
# First release of SRLP does not have "class Network(object)"
# Adding it here after grlp v2.0.0 beta release and updating for sand-bed river networks SRLP, by NI.
# Commentted sections might not be updated.
...
class Network(object):
        ...
        for lp in self.list_of_LongProfile_objects:
            ...
            if len(lp.upstream_segment_IDs) == 0:
                ...
                for ID in lp.upstream_segment_IDs:
                    ...
                    # Same as above.                    # modified to 5/6 for sand-bed rivers SRLP, by NI.
                    C0 = upseg.k_Qs * upseg.intermittency \
                            / ((1-upseg.lambda_p) * upseg.sinuosity**(5/6.)) \
                            * self.dt
                    ...
                    # Same as above.                    # modified to (-1/6) for sand-bed rivers, by NI.
                    # This is specific for the upstream reach
                    ## dzdx_0_16 calculation was modified based on the -1/6 power for sand-bed rivers SRLP, by NI and AW.
                    dz = lp.z_ext[_relative_id][1] - lp.z_ext[_relative_id][0]
                    if dz == 0:
                        dzdx_0_16 = 0
                    else:
                        dzdx_0_16 = ( np.abs( dz )
            ...
            S0 = - np.sign(_Q0) * _sinuosity * \
              ( np.abs(Q_s_0) /
                ( _k_Qs * _intermittency
                      * np.abs(_Q0)) )**(6/5.)      # modified to 6/5 for sand-bed rivers SRLP, by NI.
    ...
    def initialize(self,
        ...
        for lp in segments:
            ...
            lp.basic_constants()
            # !!!!!!!!! moving below line to after Manning's n is set, by NI and AW. 
            # !!!!!!!! #lp.sediment_lumped_constants()       # bedload_ to sediment_lumped_constants, modified for sand-bed rivers SRLP, by NI.
            ...
            # Setting sand-bed rivers SRLP parameters, by NI and AW.
            # Force uniform scalar values for now
            lp.set_D( D )
            lp.set_Mannings_roughness( n )
            lp.set_Darcy_Weisbach_friction( C_f )
            lp.set_tau_crit_bank( tau_crit_bank )

            # Now setting constants (have prereqs for SRLP set), by NI and AW.
